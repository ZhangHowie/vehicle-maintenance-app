...
- **导出 JSON 包含全部车辆（不止当前选中的一辆）及其保养/油耗记录，车辆封面图片也会一并内嵌进这份 JSON**（不是只存一个图片链接），换一台机器、换一个全新的数据卷部署，只靠这一份 JSON 文件就能把车辆、记录、封面图片完整恢复回来。
- **导入是新增而不是覆盖**：可以反复导入，不会清空已有数据；只要文件里有几辆车就会导入几辆，不会出现"导出时有 2 辆车导入后只剩 1 辆"的情况——整批导入是一次数据库事务，中途任何一条数据有问题都会整体失败回滚并给出具体错误，不会留下部分导入的残缺数据。
- **导出文件带版本号（`schemaVersion`）**：老版本导出的备份文件依然可以导入到新版本系统（自动兼容处理）；如果备份文件的版本比当前系统还新，会直接拒绝导入并提示先升级系统，而不是静默丢字段导致数据异常。详见 `docs/API.md`「数据导入导出」章节。
- `backup` 容器每天（可通过 `.env` 中 `BACKUP_INTERVAL_SECONDS` 调整）自动执行 `pg_dump`，备份文件保存在 Docker 卷 `backup-data` 中，默认保留 14 天（`BACKUP_KEEP_DAYS`）。这是数据库层面的完整备份（含所有账号数据），跟"账户设置"里的 JSON 导出（单账号、可跨环境迁移）是两种互补的备份方式。
- 手动恢复数据库备份：
