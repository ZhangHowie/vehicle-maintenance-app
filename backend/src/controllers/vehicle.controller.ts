...
import { buildVehicleWhere, canAccessVehicleOwnedBy } from "../utils/scope";
...
async function ensureOwnership(vehicleId: string, userId: string, userRole: Role | undefined) {
...
export async function getVehicle(req: Request, res: Response) {
  const vehicle = await ensureOwnership(req.params.id, req.userId!, req.userRole);
  res.json(vehicle);
}

export async function updateVehicle(req: Request, res: Response) {
  await ensureOwnership(req.params.id, req.userId!, req.userRole);
